document.addEventListener("DOMContentLoaded", () => {
  let allCars = []; // Store all cars data
  let currentFilter = 'Disponible'; // Default filter
    ...
    filterButtons.forEach(button => {
      button.addEventListener('click', (e) => {
        ...
        filterButtons.forEach(btn => {
          ...
          btn.classList.add('bg-gray-300', 'hover:bg-gray-400', 'text-gray-700');
        });
        ...
        // Set current filter
        if (button.id === 'filter-disponible') {
          currentFilter = 'Disponible';
        } else if (button.id === 'filter-sur-commande') {
          currentFilter = 'sur commande';
        }
  ...
  function loadCars() {
    ...
    fetch('https://smay.onrender.com/api/cars/')
      .then(res => res.json())
      .then(cars => {
        allCars = cars;
        filterAndRenderCars();
  ...
  function filterAndRenderCars() {
    ...
    const filteredCars = allCars.filter(car => {
      const availability = car.basic_details?.Availability || '';
      return availability.toLowerCase() === currentFilter.toLowerCase();
    });
  ...
  function createAvailabilityBadge(availability) {
    ...
    const badgeText = isAvailable 
      ...
      : (window.getTranslation ? window.getTranslation('on-order') || 'Sur Commande' : 'Sur Commande');
